...
from django.shortcuts import render
from .forms import DiseasePredictionForm  # Import the form
...
def index(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            choice_input = input_to_model(choice)
            prediction = algo_predict(algo, choice_input)
            dict_result = get_disease_data(name, algo, prediction[0])

            return render(request, 'report.html', dict_result)
    else:
        form = DiseasePredictionForm()

    return render(request, 'index.html', {'form': form})  # Ensure you have a `form.html` template
